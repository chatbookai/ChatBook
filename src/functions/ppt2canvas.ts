export function Ppt2Canvas(_canvas: string | HTMLCanvasElement, imageCrossOrigin?: string) {
    let canvas: HTMLCanvasElement;
    if (typeof _canvas === 'string') {
        canvas = document.getElementById(_canvas) as HTMLCanvasElement;
    } else {
        canvas = _canvas;
    }
    ...
    let pageIndex = 0;
    let templateHandle: any = null;

    const drawPptx = async (pptxObj: any, pageIdx: number) => {
        idMap = {};
        imageCache = {};
        pptx = pptxObj;
        pageIndex = pageIdx;
        ...
        recursion(page.children, (obj: any) => {
            idMap[obj.id] = obj;
    ...
    const resetSize = (width: number, height: number) => {
        canvas.width = width * 2;
        canvas.height = height * 2;
        canvas.style.width = width + 'px';
        canvas.style.height = height + 'px';
        if (pptx) {
            drawPptx(pptx, pageIndex);
        }
    };

    const drawElement = async (obj: any) => {
    ...
    const calcTextHeight = (obj: any) => {
        ...
        for (let i = 0; i < obj.children.length; i++) {
            ...
            if (wordWrap) {
                let lineNum = Number((totalTextWidth - 1) / (obj.extInfo.property.anchor[2] - wInset)) + 1;
    ...
    const drawTextP = async (textObj: any, p: any, isVertical: boolean, marginTop: number, pIdx: number) => {
        if (!p.children) {
            return 0;
        }
        let fontList: string[] = [];
        ...
        for (let i = 0; i < p.children.length; i++) {
            ...
            let property = r.extInfo.property;
            let fontSize = property.fontSize || 16;
            let drawString = (s: string, x: number, y: number) => {
    ...
    const drawTableColumn = async (property: any) => {
    ...
    const drawDiagram = async (obj: any) => {
        let property = obj.extInfo.property;
        ctx.save();
    ...
    const drawContainer = async (obj: any) => {
        let property = obj.extInfo.property;
    ...
    const recursionGroupChildren = (children: any[], fn: (c: any) => void) => {
        if (!children) {
            return;
        }
    ...
    const drawConnector = async (obj: any) => {
        let property = obj.extInfo.property;
        ctx.save();
        shapeHandle(property);
        await drawGeometry(property);
    ...
    const drawGraphicFrame = async (obj: any) => {
        let property = obj.extInfo.property;
        ctx.save();
        shapeHandle(property);
        if (templateHandle) {
    ...
    const drawTableRow = async (table: any, row: any) => {
    ...
    const drawGeometry = async (property: any) => {
    ...
    const drawPaths = async (paths: any[], property: any, anchor: number[], real_anchor: number[]) => {
	...
	type GradientType = 'linear' | 'circular' | 'rectangular' | 'shape';
	type Alignment = 'CENTER' | 'OTHER';
	type DuoTonePrst = 'white' | 'black';
	type PatternPrst = 'pattern1' | 'pattern2';
	type PaintType = 'noFill' | 'color' | 'bgFill' | 'groupFill' | 'gradient' | 'texture' | 'pattern';
	...
	interface Property {
		...
		interiorAnchor?: Anchor;
		placeholder?: { type: string };
	}

	interface Element {
		extInfo: { property?: Property };
		children?: Element[];
		noDraw?: boolean;
	}
	...
	interface Context {
		...
		groupFlipX?: number;
		groupFlipY?: number;
		groupFillStyle?: Paint;
		interior?: { scaleX: number; scaleY: number; tx: number; ty: number };
		_interior?: any;
	}

	function shapeHandle(property: Property, isText: boolean = false): void {
		let anchor = property.anchor;
		if (!anchor) {
			return;
		}
	...
	async function drawBackground(background: Background | null): Promise<void> {
		if (!background) {
	...
	async function drawSlideMaster(slideMaster: SlideMaster, slideLayout: SlideLayout | null, placeholder: { [key: string]: Property }): Promise<void> {
		recursion(slideMaster.children, (obj: Element) => {
			if (obj.extInfo.property && obj.extInfo.property.placeholder) {
				obj.noDraw = true;
	...
	async function drawSlideLayout(slideLayout: SlideLayout, placeholder: { [key: string]: Property }): Promise<void> {
		recursion(slideLayout.children, (obj: Element) => {
			if (obj.extInfo.property && obj.extInfo.property.placeholder) {
				obj.noDraw = true;
				placeholder[obj.extInfo.property.placeholder.type] = obj.extInfo.property;
			}
	...
	function recursion(children: Element[] | undefined, fn: (obj: Element) => void): void {
		if (!children) {
			return;
		}
		for (let i = 0; i < children.length; i++) {
	...
	function doShadow(ctx: Context, shadow: Shadow | null): void {
		if (!shadow) {
			return;
		}
	...
	async function toPaint(paint: Paint | null, anchor: Anchor, isBackground: boolean, defaultColor: string = ''): Promise<string | CanvasGradient | CanvasPattern> {
		return new Promise((resolve, reject) => {
			if (!paint) {
				...
				let gradient: any = paint.gradient;
				...
				let imgCtx: any = imgCanvas.getContext('2d');
	...
	function createTexturePattern(img: HTMLImageElement, texture: Texture, anchor: Anchor, isBackground: boolean): CanvasPattern | null {
		let width = anchor[2];
		let height = anchor[3];
		...
		let patternCtx: any = patternCanvas.getContext('2d');
		...
		if (texture.duoTone && texture.duoTone.length > 0 && texture.duoTonePrst) {
			try {
				let color: any = texture.duoTone[0].realColor;
				let r = (color >> 16) & 255;
				let g = (color >> 8) & 255;
				let b = (color >> 0) & 255;
	...
	function toColor(colorObj: ColorObject | null, defaultColor?: string): string {
		if (colorObj == null || (colorObj.color == null && colorObj.realColor == null)) {
			return defaultColor || 'transparent';
		}
		let color: any = colorObj.realColor != null ? colorObj.realColor : colorObj.color;
		let r = (color >> 16) & 255;
		let g = (color >> 8) & 255;
		let b = (color >> 0) & 255;
	...
	function toColorValue(r: number, g: number, b: number, a: number): number {
		a = (a == null ? 255 : a);
		return (a & 255) << 24 | (r & 255) << 16 | (g & 255) << 8 | (b & 255) << 0;
	}

	function loadImage(src: string, anonymous: boolean): Promise<HTMLImageElement | undefined> {
	...
	function value2px(v: number, isEmu: boolean): number {
		if (isEmu) {
			return +v / 12700;
		} else {
			return +v;
	...
	function value2emu(v: number): number {
		return +v * 12700;
	}

	function getTfx(x: number, div: boolean): number {
		if (div) {
	...
	function getTfy(y: number, div: boolean): number {
		if (div) {
	...
	function splitWords(str: string): { text: string, word: boolean }[] {
		if (!str) {
			return [];
		}
		let pattern = /^[0-9a-zA-Z]+$/;
		let array: { text: string, word: boolean }[] = [];
		let item = '';
		let math = false;
	...
	function text(obj: any): string | null {
		if (obj == null) {
			return null;
		}
		let text = obj.text;
	...
	function setTemplateHandle(_templateHandle: any) {
