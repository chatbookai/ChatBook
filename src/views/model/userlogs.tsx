...
const Logs = () => {
  ...
  const fetchData = async function (paginationModel: any) {
    if (auth && auth.user) {
      ...
      const RS = await axios.post(authConfig.backEndApiChatBook + '/api/user/getuserlogsall', data, { headers: { Authorization: auth.user.token, 'Content-Type': 'application/json'} }).then(res=>res.data)
      console.log("RS", RS)
      setStore(RS)  
    }
  }
