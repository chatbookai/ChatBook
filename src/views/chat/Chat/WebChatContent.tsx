...
import { GetWebChatById } from 'src/functions/ChatBook'
...
const ChatContent = (props: any) => {
  ...
  const LLMS = GetWebChatById(chatId)
    ...
    return (
      ...
      >
        ...
        >
          {LLMS && LLMS[0] && LLMS[0].avatar ? (
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                ...
                >
                  <MuiAvatar src={LLMS[0].avatar} alt={chatName} sx={{ width: '2.375rem', height: '2.375rem' }} />
                </Badge>
                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                  <Typography sx={{ fontWeight: 500, fontSize: '0.875rem' }}>{chatName}</Typography>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            {mdAbove ? <Fragment></Fragment> : null}
            <OptionsMenu
              menuProps={{ sx: { mt: 2 } }}
