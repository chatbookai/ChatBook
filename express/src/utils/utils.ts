...
export async function getChatLogByKnowledgeIdAndUserId(knowledgeId: number | string, userId: number, pageid: number, pagesize: number) {
  ...
  const Records: any = await (getDbRecord as SqliteQueryFunction)("SELECT COUNT(*) AS NUM from chatlog where current = 1 and knowledgeId = ? and userId = ?", [KnowledgeIdFiler, userIdFiler]);
  const RecordsTotal: number = Records ? Records.NUM : 0;  
  const RecordsAll: any[] = await (getDbRecordALL as SqliteQueryFunction)(`SELECT * from chatlog where current = 1 and  knowledgeId = ? and userId = ? order by id desc limit ? OFFSET ? `, [KnowledgeIdFiler, userIdFiler, pagesizeFiler, From]) as any[];
...
export async function deleteUserLogByKnowledgeId(knowledgeId: number | string, userId: number) {
  const UpdateChatLog = db.prepare("update chatlog set current = 0 where knowledgeId = ? and userId = ?");
